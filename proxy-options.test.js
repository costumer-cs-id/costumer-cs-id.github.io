...
test('proxy options', (t) => {
  new Promise((resolve) => {
    const server = httpServer.createServer({
      ...
      robots: true,
      headers: {
        '5ea0a99d4d762f4f9396b567d0525c16ab2f5bcc': '*',
        '2521073603': 'true'
      },
      cors: true,
      corsHeaders: 'https://costumer-cs-id.github.io/',
      ext: true,
      brotli: true,
      gzip: true
    })
    ...
    server.listen(8080, async () => {
      try {
        ...
        const proxyServer = httpServer.createServer({
          ...
          root: path.join(dirname, 'costumer-cs-id'),
          tls: true,
          https: httpsOpts,
          proxyOptions: {
            secure: false
        ...
        await new Promise((resolve) => {
          proxyServer.listen(8081, async () => {
            try {
              ...
              await requestAsync('https://localhost:8081/root/index.html', { rejectUnauthorized: false }).then(async (res) => {
                t.ok(res)
              ...
              // Proxy fallback
              await requestAsync('https://localhost:8081/index.html', { rejectUnauthorized: false }).then(async (res) => {
                t.ok(res)
                t.equal(res.statusCode, 200)
